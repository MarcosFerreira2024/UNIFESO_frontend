...
function PedidosInfoModal({isOpen,setOpen,quantidade,total,descricao,imagem,nome}:ModalProps) {

    const totalFormatado = (total/100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    ...
    <div onClick={()=> setOpen(false)} className='absolute top-0 text-main bg-black/20 z-100 right-0 left-0 bottom-0 flex justify-center items-center min-h-screen'>
        ...
        <div onClick={(e)=>e.stopPropagation()} className='bg-main-bg flex  flex-col border-2 border-stroke rounded-sm dropShadow '>
            ...
            <div className='flex flex-row  gap-5 p-5'>
                <div className='flex flex-col justify-between gap-5'>
                    ...
                    <div className='flex flex-col gap-2'>
                        <p className='text-2xl '>Quantidade de Itens: <span className='font-bold'>{quantidade}</span></p>

                        <p className='text-2xl '>Total a pagar: <span className='font-bold'>{totalFormatado}</span></p>

                        <Button label='Confirmar'>Confirmar</Button>
                    </div>
                </div>
...
export default PedidosInfoModal
